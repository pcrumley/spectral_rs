...
arr1 = rng.random(24)
fft = np.fft.fft(arr1)
print("let input: Vec<Float> = vec![", " ".join(f"Complex::new({elm}, 0.0)," for elm in arr1), '];')
print("let out: Vec<Float> = vec![", " ".join(f"Complex::new({elm.real}, {elm.imag})," for elm in fft), '];')
